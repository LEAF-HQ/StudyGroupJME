...
#include "LEAF/StudyGroup/include/StudyGroupHists.h"

using namespace std;

class StudyGroupTool : public BaseTool {

public:
  // Constructors, destructor
  StudyGroupTool(const Config & cfg);
  ~StudyGroupTool() = default;
  void ProcessDataset(const Config & cfg) override {LoopEvents<StudyGroupTool, StudyGroupEvent>(cfg, event, *this);};
...
StudyGroupTool::StudyGroupTool(const Config & cfg) : BaseTool(cfg){

  event = new StudyGroupEvent();
  event->reset();
...
void StudyGroupTool::book_histograms(vector<TString> tags){
  for(const TString & tag : tags){
    TString mytag = tag+"_General";
    book_HistFolder(mytag, new StudyGroupHists(mytag));
    mytag = tag+"_Jets";
...
void StudyGroupTool::fill_histograms(TString tag){
  TString mytag = tag+"_General";
  HistFolder<StudyGroupHists>(mytag)->fill(*event);
  mytag = tag+"_Jets";
